// 使われた単語のリスト
const usedWords = [];
// ファイルから読み込んだ単語リスト (ここでは例として固定の単語)
let availableWords = `
フシギダネ
フシギソウ
フシギバナ
...
document.getElementById('submitBtn').addEventListener('click', function() {
    ...
    const lastChar = usedWords.length > 0 ? usedWords[usedWords.length - 1].slice(-1) : null;
...
function getBotResponse(userWord) {
    ...
    const botWord = availableWords.find(word => word[0] === lastChar && !usedWords.includes(word));
    return botWord || null;
}
